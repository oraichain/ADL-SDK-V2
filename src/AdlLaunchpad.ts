...
export class AdlLaunchpad {
    ...
    async getUsersInfoByPool(
        ...
        let listStakeInfoPda = [];
        for (let user of users) {
            const [stakerInfoPda] = PublicKey.findProgramAddressSync(
                [Buffer.from('staker_info_v2'), stakingVaultPda.toBuffer(), user.toBuffer()],
                this._program.programId
            );
            listStakeInfoPda.push(stakerInfoPda);
        }
        const stakerInfoAccounts = await this._program.account.stakerInfoV2.fetchMultiple(listStakeInfoPda);

        return stakerInfoAccounts;
    }
